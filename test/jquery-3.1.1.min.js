...
!(function (a, b) {
  ...
  let E;
  const F = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]+))$/;
  const G = (r.fn.init = function (a, b, c) {
    let e;
    let f;
    if (!a) return this;
  ...
  r.Callbacks = function (a) {
    a = typeof a === 'string' ? L(a) : r.extend({}, a);
    let b;
    let c;
    let d;
    let e;
    ...
    r.fn.extend({
      data(a, b) {
        ...
        if (void 0 === a) {
          if (
            this.length &&
            ((e = W.get(f)), f.nodeType === 1 && !V.get(f, 'hasDataAttrs'))
          ) {
            c = g.length;
            while (c--)
              g[c] &&
  ...
  const _ = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source;
  const aa = new RegExp(`^(?:([+-])=|)(${_})([a-z%]*)$`, 'i');
  ...
  function ea(a, b, c, d) {
    ...
    let k = (r.cssNumber[b] || (j !== 'px' && +i)) && aa.exec(r.css(a, b));
    if (k && k[3] !== j) {
      (j = j || k[3]), (c = c || []), (k = +i || 1);
      ...
      (d = a[f]),
        d.style &&
          ...
          b
            ...
            : c !== 'none' && ((e[f] = 'none'), V.set(d, 'display', c)));
  ...
  const ia = /^(?:checkbox|radio)$/i;
  const ja = /<([a-z][^\/\0>\x20\t\r\n\f]+)/i;
  const ka = /^$|\/(?:java|ecma)script/i;
  const la = {
  ...
  const ra = /^key/;
  const sa = /^(?:mouse|pointer|contextmenu|drag|drop)|click/;
  const ta = /^([^.]*)(?:\.(.+)|)/;
  function ua() {
    return !0;
  }
  ...
  (r.event = {
    ...
    dispatch(a) {
      ...
      const j = (V.get(this, 'events') || {})[b.type] || [];
    ...
    r.each(
      {
        ...
        which(a) {
          const b = a.button;
          return a.which == null && ra.test(a.type)
            ? a.charCode != null
              ? a.charCode
              : a.keyCode
  ...
  const za = /<script|<style|<link/i;
  const Aa = /checked\s*(?:[^=]|=\s*.checked.)/i;
  const Ba = /^true\/(.*)/;
  const Ca = /^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*$/g;
  function Da(a, b) {
    return r.nodeName(a, 'table') &&
      r.nodeName(b.nodeType !== 11 ? b : b.firstChild, 'tr')
  ...
  function Ia(a, b, c, d) {
    ...
    if (s || (m > 1 && typeof q === 'string' && !o.checkClone && Aa.test(q)))
  ...
  function Ja(a, b, c) {
    for (var d, e = b ? r.filter(b, a) : a, f = 0; (d = e[f]) != null; f++)
      c || d.nodeType !== 1 || r.cleanData(ma(d)),
        d.parentNode &&
    ...
    r.fn.extend({
      ...
      empty() {
        for (var a, b = 0; (a = this[b]) != null; b++)
          a.nodeType === 1 && (r.cleanData(ma(a, !1)), (a.textContent = ''));
        return this;
      },
      clone(a, b) {
        return (
  ...
  const Ka = /^margin/;
  const La = new RegExp(`^(${_})(?!px)[a-z%]+$`, 'i');
  const Ma = function (b) {
  ...
  const Pa = /^(none|table(?!-c[ea]).+)/;
  const Qa = { position: 'absolute', visibility: 'hidden', display: 'block' };
  const Ra = { letterSpacing: '0', fontWeight: '400' };
  const Sa = ['Webkit', 'Moz', 'ms'];
  const Ta = d.createElement('div').style;
  function Ua(a) {
    if (a in Ta) return a;
  ...
  function Wa(a, b, c, d, e) {
    ...
    for (
      ...
      c === 'margin' && (g += r.css(a, c + ba[f], !0, e)),
        d
          ? (c === 'content' && (g -= r.css(a, `padding${ba[f]}`, !0, e)),
            c !== 'margin' && (g -= r.css(a, `border${ba[f]}Width`, !0, e)))
          : ((g += r.css(a, `padding${ba[f]}`, !0, e)),
            c !== 'padding' && (g += r.css(a, `border${ba[f]}Width`, !0, e)));
    return g;
  }
  function Xa(a, b, c) {
    let d;
    let e = !0;
    const f = Ma(a);
    const g = r.css(a, 'boxSizing', !1, f) === 'border-box';
    if (
    ...
    r.each(['height', 'width'], function (a, b) {
      r.cssHooks[b] = {
        ...
        set(a, c, d) {
          ...
          const f = d && Ma(a);
          const g =
            d && Wa(a, b, d, r.css(a, 'boxSizing', !1, f) === 'border-box', f);
          return (
            g &&
              (e = aa.exec(c)) &&
              (e[3] || 'px') !== 'px' &&
  ...
  (r.Tween = Ya),
    (Ya.prototype = {
      ...
      cur() {
        const a = Ya.propHooks[this.prop];
        return a && a.get ? a.get(this) : Ya.propHooks._default.get(this);
      },
      run(a) {
        let b;
        const c = Ya.propHooks[this.prop];
        return (
  ...
  function fb(a, b, c) {
    ...
    const n = {};
    const o = a.style;
    let p = a.nodeType && ca(a);
    let q = V.get(a, 'fxshow');
    c.queue ||
  ...
  function hb(a, b, c) {
    ...
    let e;
    let f = 0;
    const g = hb.prefilters.length;
    const h = r.Deferred().always(function () {
      delete i.elem;
    });
    var i = function () {
      if (e) return !1;
  ...
  const kb = /^(?:input|select|textarea|button)$/i;
  const lb = /^(?:a|area)$/i;
  r.fn.extend({
    prop(a, b) {
  ...
  r.fn.extend({
    ...
    hasClass(a) {
      ...
      b = ` ${a} `;
      while ((c = this[d++]))
        if (c.nodeType === 1 && ` ${mb(nb(c))} `.indexOf(b) > -1) return !0;
      return !1;
    },
  });
  const ob = /\r/g;
        ...
        if (e)
          return (
            ...
            b && 'get' in b && void 0 !== (c = b.get(e, 'value'))
              ? c
              : ((c = e.value),
                typeof c === 'string' ? c.replace(ob, '') : c == null ? '' : c)
          );
      }
    },
  }),
  ...
  const pb = /^(?:focusinfocus|focusoutblur)$/;
  r.extend(r.event, {
    trigger(b, c, e, f) {
      let g;
      let h;
      ...
      r.each({ focus: 'focusin', blur: 'focusout' }, function (a, b) {
        ...
        r.event.special[b] = {
          setup() {
            ...
            const e = V.access(d, b);
            e || d.addEventListener(a, c, !0), V.access(d, b, (e || 0) + 1);
          },
          teardown() {
            const d = this.ownerDocument || this;
            const e = V.access(d, b) - 1;
            e
              ? V.access(d, b, e)
  ...
  const tb = /\[\]$/;
  const ub = /\r?\n/g;
  const vb = /^(?:submit|button|image|reset|file)$/i;
  const wb = /^(?:input|select|textarea|keygen)/i;
  function xb(a, b, c, d) {
    let e;
    if (r.isArray(b))
      r.each(b, function (b, e) {
        c || tb.test(a)
          ? d(a, e)
          : xb(`${a}[${typeof e === 'object' && e != null ? b : ''}]`, e, c, d);
      });
  ...
  const Ab = /([?&])_=[^&]*/;
  const Bb = /^(.*?):[ \t]*([^\r\n]*)$/gm;
  const Cb = /^(?:about|app|app-storage|.+-extension|file|res|widget):$/;
  const Db = /^(?:GET|HEAD)$/;
  const Eb = /^\/\//;
  const Fb = {};
  const Gb = {};
  const Hb = '*/'.concat('*');
  const Ib = d.createElement('a');
  Ib.href = qb.href;
  function Jb(a) {
    return function (b, c) {
  ...
  const Ob = { 0: 200, 1223: 204 };
  let Pb = r.ajaxSettings.xhr();
